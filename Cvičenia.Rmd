...
```{r, fig.width=8, fig.height=5, fig.align='center'}
fit$All <- arima(log(dat$train$log),
  order = c(1, 0, 0),
...
# dat$train[paste0("All_", names(fit$All))] <-
#   (eval(fit$All$call$x) - fit$All$residuals) |>
#   lapply(exp) |>
#   as.data.frame()
```
...
dat$valid <- dat$valid |>
  transform(
    multi = exp(exp(tmp$pred)), # stredná hodnota
    multi_lo = exp(exp(tmp$pred - 2 * tmp$se)),
    multi_hi = exp(exp(tmp$pred + 2 * tmp$se))
  )
...
# BUG: Toto nefunguje!!! neviem prečo...
# naše modely
tmp <- list(
...
dat$valid <- dat$valid |>
  transform(
    single = exp(exp(tmp$pred)), # stredná hodnota
    single_lo = exp(exp(tmp$pred - 2 * tmp$se)),
    single_hi = exp(exp(tmp$pred + 2 * tmp$se))
  )
...
Z chýb vidíme, že naivné sezónne predpovede sú omnoho presnejšie ako obyčajné naivné. Zároveň sa ukázalo, že náš model nevykazuje väčšiu presnosť ako naivné sezónne predpovede.
